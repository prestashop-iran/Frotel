...
include(dirname(__FILE__).'/fa.php'); //me
...
class AdminProducts extends AdminTab
{
			...
			if (isset($id) AND !empty($id))
			{
				...
				if (Validate::isLoadedObject($object))
				{
					...
					if ($object->update())
					{
                        ...
                        //me
                        elseif((!Tools::isEmpty(Configuration::get('FROTEL_USERNAME')) AND !Tools::isEmpty(Configuration::get('FROTEL_PASSWORD')) AND Configuration::get('FROTEL_BAZARYAB_POSSIBILITY')==1) AND (!Validate::isUnsignedInt(Tools::getValue('fro_percent')) OR Tools::getValue('fro_percent')> 99))
                            $this->_errors[] = Tools::displayError($this->l('Inter valid percent'));
	...
	private function _displayDraftWarning($active)
	{
		return '<div class="warn draft" style="'.($active ? 'display:none' : '').'">
				<p>
				<span style="float: '.$this->left.'">
				<img src="../img/admin/warn2.png" />
				'.$this->l('Your product will be saved as draft').'
				</span>
				<span style="float:'.$this->right.'"><a href="#" class="button" style="display: block" onclick="submitAddProductAndPreview()" >'.$this->l('Save and preview').'</a></span>
	...
	public function displayForm($isMainTab = true)
	{
		...
		<link type="text/css" rel="stylesheet" href="../css/tabpane'.($this->isRTL ? '_rtl' : '').'.css" />
	...
	private function _getCustomizationFieldIds($labels, $alreadyGenerated, $obj)
	{
		...
		return implode('آ¤', $customizableFieldIds);
	}
	...
	private function _displayLabelField(&$label, $languages, $defaultLanguage, $type, $fieldIds, $id_customization_field)
	{
		...
		foreach ($languages as $language)
		{
			...
			echo '<div class="lang_'.$language['id_lang'].'" id="'.$fieldName.'" style="display: '.((int)($language['id_lang']) == (int)($defaultLanguage) ? 'block' : 'none').'; clear: '.$this->left.'; float: '.$this->left.'; padding-bottom: 4px;">
						<div style="margin-'.$this->right.': 6px; float:'.$this->left.'; text-align:'.$this->right.';">#'.(int)($id_customization_field).'</div><input type="text" name="'.$fieldName.'" value="'.htmlentities($text, ENT_COMPAT, 'UTF-8').'" style="float: '.$this->left.'" />
					</div>';
		}

		$required = (isset($label[(int)($language['id_lang'])])) ? $label[(int)($language['id_lang'])]['required'] : false;
		echo '</div>
				<div style="margin: '.($this->isRTL ? '3px 3px 0 0' : '3px 0 0 3px').'; font-size: 11px">
					<input type="checkbox" name="require_'.$type.'_'.(int)($id_customization_field).'" id="require_'.$type.'_'.(int)($id_customization_field).'" value="1" '.($required ? 'checked="checked"' : '').' style="float: '.$this->left.'; margin: 0 4px"/><label for="require_'.$type.'_'.(int)($id_customization_field).'" style="float: none; font-weight: normal;"> '.$this->l('required').'</label>
				</div>';
	}
	...
	function displayFormCustomization($obj, $languages, $defaultLanguage)
	{
		...
		echo '
			...
			<table cellpadding="5" style="width:100%">
				<tr>
					<td style="width:150px;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;vertical-align:top;" valign="top">'.$this->l('File fields:').'</td>
				...
				<tr>
					<td style="width:150px;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;vertical-align:top;" valign="top">'.$this->l('Text fields:').'</td>
					<td style="padding-bottom:5px;">
	...
	function displayFormAttachments($obj, $languages, $defaultLanguage)
	{
		...
		'.($obj->id ? '<input type="hidden" name="id_'.$this->table.'" value="'.$obj->id.'" />' : '').'
			<fieldset><legend><img src="../img/t/AdminAttachments.gif" />'.$this->l('Attachment').'</legend>
				...
				<div class="margin-form">';
		foreach ($languages as $language)
			echo '	<div id="attachment_name_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $defaultLanguage ? 'block' : 'none').'; float: '.$this->left.';">
		...
		$this->displayFlags($languages, $defaultLanguage, 'attachment_nameآ¤attachment_description', 'attachment_name');
		echo '	</div>
				<div class="clear">&nbsp;</div>
				<label>'.$this->l('Description:').' </label>
				<div class="margin-form">';
		foreach ($languages as $language)
			echo '	<div id="attachment_description_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $defaultLanguage ? 'block' : 'none').'; float: '.$this->left.';">
		...
		$this->displayFlags($languages, $defaultLanguage, 'attachment_nameآ¤attachment_description', 'attachment_description');
		echo '	</div>
				<div class="clear">&nbsp;</div>
		...
		<table>
			...
			echo '	</select><br /><br />
				...
				<td style="padding-'.$this->left.':20px;">
	...
	function displayFormInformations($obj, $currency)
	{
		...
		if (isset($obj->id))
		{
			...
			echo '
			<a href="index.php?tab=AdminCatalog&id_product='.$obj->id.'&deleteproduct&token='.$this->token.'" style="float:'.$this->right.';"
		...
		echo '
			<hr class="clear"/>
			<br />
				<table cellpadding="5" style="width: 50%; float: '.$this->left.'; margin-'.$this->right.': 20px; border-'.$this->right.': 1px solid #E0D0B1;">
					<tr>
						<td class="col-left">'.$this->l('Name:').'</td>
						<td style="padding-bottom:5px;" class="translatable">';
		foreach ($this->_languages as $language)
			echo '		<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: '.$this->left.';">
		...
		echo '		</td>
					...
					<tr>
						...
						<td style="padding-bottom:5px;">
							<input size="55" type="text" name="reference" value="'.htmlentities($this->getFieldValue($obj, 'reference'), ENT_COMPAT, 'UTF-8').'" style="width: 130px; margin-'.$this->right.': 44px;" />
					...
					<tr>
						...
						<td style="padding-bottom:5px;">
							<input size="55" type="text" name="supplier_reference" value="'.htmlentities($this->getFieldValue($obj, 'supplier_reference'), ENT_COMPAT, 'UTF-8').'" style="width: 130px; margin-'.$this->right.': 44px;" />
							<span class="hint" name="help_box">'.$this->l('Special characters allowed:').' .-_#\<span class="hint-pointer">&nbsp;</span></span>
						</td>
					</tr>
					<tr>
						...
						<td style="padding-bottom:5px;">
							<input size="55" maxlength="13" type="text" name="ean13" value="'.htmlentities($this->getFieldValue($obj, 'ean13'), ENT_COMPAT, 'UTF-8').'" style="width: 130px; margin-'.$this->right.': 5px;" /> <span class="small">'.$this->l('(Europe, Japan)').'</span>
						</td>
					</tr>
					<tr>
						<td class="col-left">'.$this->l('UPC:').'</td>
						<td style="padding-bottom:5px;">
							<input size="55" maxlength="12" type="text" name="upc" value="'.htmlentities($this->getFieldValue($obj, 'upc'), ENT_COMPAT, 'UTF-8').'" style="width: 130px; margin-'.$this->right.': 5px;" /> <span class="small">'.$this->l('(US, Canada)').'</span>
						</td>
					</tr>
					<tr>
						<td class="col-left">'.$this->l('Location (warehouse):').'</td>
						<td style="padding-bottom:5px;">
							<input size="55" type="text" name="location" value="'.htmlentities($this->getFieldValue($obj, 'location'), ENT_COMPAT, 'UTF-8').'" style="width: 130px; margin-'.$this->right.': 44px;" />
						</td>
					</tr>
					<tr>
                    ...
                    if(!Tools::isEmpty(Configuration::get('FROTEL_USERNAME')) AND !Tools::isEmpty(Configuration::get('FROTEL_PASSWORD'))){
                        $this->_ShowFrotelTab();
                        echo '<tr>
                                <td colspan="2">
			                     <div style="margin: 10px 10px 0 0;">
                                    <h4 id="frotel-tab" style="background: #FEFEFE; border-radius: 3px 3px 0 0; border: 1px solid #9a9aae; border-bottom: 0px;padding: 3px 12px; float: left;  z-index: 3; margin: -20px 0px -3px 60px;color: #727272;"> '.$this->l('Frotel').' </h4>

                                    <fieldset class="fro_fset">
                                        <table border="0">
                                            <tr>
                                                <td style="min-width: 200px;"><label class="fro_label" for="weight">'.$this->l('weight in gr :').' <span>*</span></label></td>
                                                <td><input class="fro_textbox" name="weight" id=""  value="'.htmlentities($this->getFieldValue($obj, 'weight'), ENT_COMPAT, 'UTF-8').'" type="text"></td>
                                            </tr>';
                                    if(Configuration::get('FROTEL_BAZARYAB_POSSIBILITY')==1){
                                        echo '<tr>
                                                <td><label class="fro_label" for="percent">'.$this->l('percent :').'<span>*</span></label></td>
                                                <td><input class="fro_textbox" name="fro_percent" id="" value="'.htmlentities($this->_getFrotel($obj, 'percent'), ENT_COMPAT, 'UTF-8').'" type="text"></td>
                                             </tr>';
                                    }else
                                        echo '<input name="fro_percent" id="" value="0" type="hidden">';  
                                        echo '</table></fieldset></div></td></tr>';
				...
				<table cellpadding="5" style="width: 40%; float: '.$this->left.'; margin-'.$this->left.': 10px;">
					<tr>
						<td style="vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;">'.$this->l('Status:').'</td>
						<td style="padding-bottom:5px;">
							<input style="float:'.$this->left.';" onclick="toggleDraftWarning(false);showOptions(true);" type="radio" name="active" id="active_on" value="1" '.($this->getFieldValue($obj, 'active') ? 'checked="checked" ' : '').'/>
							<label for="active_on" class="t"><img src="../img/admin/enabled.gif" alt="'.$this->l('Enabled').'" title="'.$this->l('Enabled').'" style="float:'.$this->left.'; padding:0px 5px 0px 5px;" />'.$this->l('Enabled').'</label>
							<br class="clear" />
							<input style="float:'.$this->left.';" onclick="toggleDraftWarning(true);showOptions(false);"  type="radio" name="active" id="active_off" value="0" '.(!$this->getFieldValue($obj, 'active') ? 'checked="checked" ' : '').'/>
							<label for="active_off" class="t"><img src="../img/admin/disabled.gif" alt="'.$this->l('Disabled').'" title="'.$this->l('Disabled').'" style="float:'.$this->left.'; padding:0px 5px 0px 5px" />'.$this->l('Disabled').($obj->active ? '' : ' (<a href="'.$preview_url.'" alt="" target="_blank">'.$this->l('View product in shop').'</a>)').'</label>
						</td>
					</tr>
					<tr id="product_options" '.(!$obj->active ? 'style="display:none"' : '').'>
						<td style="vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;">'.$this->l('Options:').'</td>
						<td style="padding-bottom:5px;">
							<input style="float: '.$this->left.';" type="checkbox" name="available_for_order" id="available_for_order" value="1" '.($this->getFieldValue($obj, 'available_for_order') ? 'checked="checked" ' : '').' onclick="if ($(this).is(\':checked\')){$(\'#show_price\').attr(\'checked\', \'checked\');$(\'#show_price\').attr(\'disabled\', \'disabled\');}else{$(\'#show_price\').attr(\'disabled\', \'\');}"/>
							<label for="available_for_order" class="t"><img src="../img/admin/products.gif" alt="'.$this->l('available for order').'" title="'.$this->l('available for order').'" style="float:'.$this->left.'; padding:0px 5px 0px 5px" />'.$this->l('available for order').'</label>
							<br class="clear" />
							<input style="float: '.$this->left.';" type="checkbox" name="show_price" id="show_price" value="1" '.($this->getFieldValue($obj, 'show_price') ? 'checked="checked" ' : '').' />
							<label for="show_price" class="t"><img src="../img/admin/gold.gif" alt="'.$this->l('display price').'" title="'.$this->l('show price').'" style="float:'.$this->left.'; padding:0px 5px 0px 5px" />'.$this->l('show price').'</label>
							<br class="clear" />
							<input style="float: '.$this->left.';" type="checkbox" name="online_only" id="online_only" value="1" '.($this->getFieldValue($obj, 'online_only') ? 'checked="checked" ' : '').' />
							<label for="online_only" class="t"><img src="../img/admin/basket_error.png" alt="'.$this->l('online only').'" title="'.$this->l('online only').'" style="float:'.$this->left.'; padding:0px 5px 0px 5px" />'.$this->l('online only (not sold in store)').'</label>
						</td>
					</tr>
					<tr>
						<td style="vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;">'.$this->l('Condition:').'</td>
						<td style="padding-bottom:5px;">
					...
					<tr>
						<td style="vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;">'.$this->l('Manufacturer:').'</td>
						<td style="padding-bottom:5px;">
		...
		echo '
					...
					<tr>
						<td style="vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;">'.$this->l('Supplier:').'</td>
						<td style="padding-bottom:5px;">
...
?>
	...
	<?php if (!$productDownload->checkFile()): ?>
				...
				<div style="padding:5px;width:50%;float:'.$this->left.';margin-'.$this->right.':20px;border-'.$this->right.':1px solid #E0D0B1">
	...
	<?php endif; // check if file exists ?>
				...
				<div id="virtual_good_more" style="<?php if (!$productDownload->id OR !$productDownload->active) echo 'display:none;' ?>padding:5px;width:40%;float:'.$this->left.';margin-'.$this->left.':10px">
...
<?php
					...
					<tr>
						...
						<td style="padding-bottom:5px;">
							...
							<span style="margin-'.$this->left.':10px">'.$this->l('The wholesale price at which you bought this product').'</span>
						</td>
					</tr>';
					echo '
					<tr>
						...
						<td style="padding-bottom:5px;">
							...
							<span style="margin-'.$this->left.':2px">'.$this->l('The pre-tax retail price to sell this product').'</span>
						</td>
					</tr>';
				...
				{
					echo '<span style="margin-'.$this->left.':10px; color:red;">'.$this->l('Taxes are currently disabled').'</span> (<b><a href="index.php?tab=AdminTaxes&token='.Tools::getAdminToken('AdminTaxes'.(int)(Tab::getIdFromClassName('AdminTaxes')).(int)($cookie->id_employee)).'">'.$this->l('Tax options').'</a></b>)';
				...
				if (Configuration::get('PS_USE_ECOTAX'))
					...
					<tr>
						...
						<td style="padding-bottom:5px;">
							...
							<span style="margin-'.$this->left.':10px">('.$this->l('already included in price').')</span>
						</td>
					</tr>';
				...
				echo '
					<tr id="tr_unit_price">
						...
						<td style="padding-bottom:5px;">
							...
							(Configuration::get('PS_TAX') && $default_country->display_tax_label ? '<span style="margin-'.$this->left.':15px">'.$this->l('or').' '.($currency->format % 2 != 0 ? ' '.$currency->sign : '').'<span id="unit_price_with_tax">0.00</span>'.($currency->format % 2 == 0 ? ' '.$currency->sign : '').' '.$this->l('per').' <span id="unity_second">'.$this->getFieldValue($obj, 'unity').'</span> '.$this->l('with tax') : '').'</span>
		...
		foreach ($this->_languages as $language)
			...
			echo '	</td>
					...
					<tr><td colspan="2" style="padding-bottom:5px;"><hr style="width:100%;" /></td></tr>
					<tr><td colspan="2">
						<span onclick="$(\'#seo\').slideToggle();" style="cursor: pointer"><img src="../img/admin/arrow.gif" alt="'.$this->l('SEO').'" title="'.$this->l('SEO').'" style="float:'.$this->left.'; margin-'.$this->right.':5px;"/>'.$this->l('Click here to improve product\'s rank in search engines (SEO)').'</span><br />
		...
		echo '		</td>
				...
				if ($postAccessories = Tools::getValue('inputAccessories'))
				{
					...
					foreach ($accessories as $accessory)
						echo htmlentities($accessory['name'], ENT_COMPAT, 'UTF-8').'آ¤';
		...
		echo '
			<script type="text/javascript">
			var iso = \''.$isoTinyMCE.'\' ;
			var pathCSS = \''._THEME_CSS_DIR_.($this->isRTL ? '/rtl/' :'').'\' ;
			var ad = \''.$ad.'\' ;
			</script>
				...
				<table cellpadding="5" style="width:100%">
					...
					<tr>
						...
						foreach ($this->_languages as $language)
						{
							...
							echo '
							<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float:'.$this->left.'; width:371px;">
	...
	function displayFormAttributes($obj, $languages, $defaultLanguage)
	{
		...
		if ($obj->id)
			...
			<table cellpadding="5" style="width:100%">
			<tr>
			  <td style="width:150px;vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;" valign="top">'.$this->l('Group:').'</td>
		  ...
		  <tr>
			  <td style="width:150px;vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;" valign="top">'.$this->l('Attribute:').'</td>
		  ...
		  <tr>
			  <td style="width:150px;vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;" valign="top">
		  ...
		  <tr><td colspan="2"><hr style="width:100%;" /></td></tr>
		  <tr>
			  <td style="width:150px;vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;">'.$this->l('Reference:').'</td>
			  <td style="padding-bottom:5px;">
				<input size="55" type="text" id="attribute_reference" name="attribute_reference" value="" style="width: 130px; margin-'.$this->right.': 44px;" />
				'.$this->l('EAN13:').'<input size="55" maxlength="13" type="text" id="attribute_ean13" name="attribute_ean13" value="" style="width: 110px; margin-'.$this->left.': 10px; margin-'.$this->right.': 44px;" />
				'.$this->l('UPC:').'<input size="55" maxlength="12" type="text" id="attribute_upc" name="attribute_upc" value="" style="width: 110px; margin-'.$this->left.': 10px;" />
				<span class="hint" name="help_box">'.$this->l('Special characters allowed:').' .-_#<span class="hint-pointer">&nbsp;</span></span>
			  </td>
		  </tr>
		  <tr>
			  <td style="width:150px;vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;">'.$this->l('Supplier Reference:').'</td>
			  <td style="padding-bottom:5px;">
				<input size="55" type="text" id="attribute_supplier_reference" name="attribute_supplier_reference" value="" style="width: 130px; margin-'.$this->right.': 44px;" />
				'.$this->l('Location:').'<input size="55" type="text" id="attribute_location" name="attribute_location" value="" style="width: 101px; margin-'.$this->left.': 10px;" />
				<span class="hint" name="help_box">'.$this->l('Special characters allowed:').' .-_#<span class="hint-pointer">&nbsp;</span></span>
			  </td>
		  </tr>
		  <tr><td colspan="2"><hr style="width:100%;" /></td></tr>
		  <tr>
			  <td style="width:150px;vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;">'.$this->l('Wholesale price:').'</td>
		  ...
		  <tr>
			  <td style="width:150px;vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;">'.$this->l('Impact on price:').'</td>
			  <td colspan="2" style="padding-bottom:5px;">
		  ...
		  <tr>
			  <td style="width:150px;vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;">'.$this->l('Impact on weight:').'</td>
		  ...
		  <tr id="tr_unit_impact">
			  <td style="width:150px;vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;">'.$this->l('Impact on unit price :').'</td>
		...
		if (Configuration::get('PS_USE_ECOTAX'))
			echo'
				  <tr>
					  <td style="width:150px;vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;">'.$this->l('Eco-tax:').'</td>
		...
		echo'
		  <tr id="initial_stock_attribute">
				<td style="width:150px;vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;" class="col-left">'.$this->l('Initial stock:').'</td>
				<td><input type="text" name="attribute_quantity" size="3" maxlength="10" value="0"/></td>
		  </tr>
		  </tr>
			<tr id="stock_mvt_attribute" style="display:none;">
				<td style="width:150px;vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;" class="col-left">'.$this->l('Stock movement:').'</td>
				<td style="padding-bottom:5px;">
					<select id="id_mvt_reason" name="id_mvt_reason">
						<option value="-1">--</option>';
			$reasons = StockMvtReason::getStockMvtReasons((int)$cookie->id_lang);
			...
			<tr>
			<td style="width:150px;vertical-align:top;text-align:'.$this->right.';padding-'.$this->right.':10px;font-weight:bold;" class="col-left">'.$this->l('Minimum quantity:').'</td>
				<td style="padding-bottom:5px;">
		  ...
		  <tr>
			...
			foreach ($images AS $image)
			{
				...
				echo '<li style="float: '.$this->left.'; width: '.$imageWidth.'px;"><input type="checkbox" name="id_image_attr[]" value="'.(int)($image['id_image']).'" id="id_image_attr_'.(int)($image['id_image']).'" />
			...
			echo '</ul>
				<img id="pic" alt="" title="" style="display: none; width: 100px; height: 100px; float: '.$this->left.'; border: 1px dashed #BBB; margin-'.$this->left.': 20px;" />
			  </td>
		  </tr>
			<tr>
		  ...
		  <tr>
			  ...
			  <td style="padding-bottom:5px;">
				<span style="float: '.$this->left.';"><input type="submit" name="submitProductAttribute" id="submitProductAttribute" value="'.$this->l('Add this combination').'" class="button" onclick="attr_selectall(); this.form.action += \'&addproduct&tabs=3\';" /> </span>
				<span id="ResetSpan" style="float: '.$this->left.'; margin-'.$this->left.': 8px; display: none;">
	...
	function displayFormFeatures($obj)
	{
		...
		if ($obj->id)
		{
			...
			foreach ($feature AS $tab)
				$ctab .= 'ccustom_'.$tab['id_feature'].'آ¤';
			$ctab = rtrim($ctab, 'آ¤');
			...
			if ($nb_feature)
			{
				...
				foreach ($feature AS $tab_features)
				{
					...
					foreach ($this->_languages as $language)
						echo '
							<div class="lang_'.$language['id_lang'].'" style="display: '.($language['id_lang'] == $this->_defaultFormLanguage ? 'block' : 'none').'; float: '.$this->left.';">
	...
	private function displayPack(Product $obj)
	{
		...
		echo '		</div>
					...
					foreach ($packItems as $packItem)
						echo $packItem->pack_quantity.' x '.$packItem->name.'آ¤';
					echo '" />
		...
		// param multipleSeparator:'||' ajoutأ© أ  cause de bug dans lib autocomplete
		echo '<script type="text/javascript">
								urlToCall = null;
								/* function autocomplete */
								$(function() {
	...
	private function addPackItem()
	{
		return '
			function addPackItem()
			{
				...
				$(\'#namePackItems\').val($(\'#namePackItems\').val() + lineDisplay + \'أ‚آ¤\');
	...
	private function delPackItem()
	{
		...
		function delPackItem(id)
		{
			...
			var nameCut = name.value.split(new RegExp(\'آ¤\', \'g\'));
     ...
     * */
     ...
     {
        global $_LANGADM;
        if(!empty($_LANGADM['AdminProducts'.md5($string)]))
            return $_LANGADM['AdminProducts'.md5($string)];
            
        elseif(!empty($_LANGADM['AdminTab'.md5($string)]))
            return $_LANGADM['AdminTab'.md5($string)];
        else
            return $string;
    }
    ...
    /**
     * Frotel tab
     * Display box in add product's form
     * */
     private function _ShowFrotelTab()
    {
        $text = '<style>
            input.fro_textbox {float: left; border-radius: 3px 3px 3px 3px; border: 1px solid rgb(187, 187, 187); font: 11px tahoma; padding: 2px 3px; text-align: left; color: rgb(85, 85, 85);margin: 2px 0;}
            input.fro_textbox:focus {border: 1px solid #AA6537;}
            fieldset.fro_fset {width: 380px; border-radius: 5px 5px 5px 5px; font: 11px tahoma; border: 1px solid #839CA9; padding: 10px; direction:rtl;background: #FEFEFE;text-align: right;box-shadow: 0px -20px 50px #F8FAFA inset;}
    ...
    /**
     * Get stored data: Percent
     * 
     * */
      private function _getFrotel($obj,$key)
     {
        $data = Db::getInstance()->getValue('SELECT '.$key.' FROM '._DB_PREFIX_.'frotel_product WHERE id_product = '.(int)$obj->id);
